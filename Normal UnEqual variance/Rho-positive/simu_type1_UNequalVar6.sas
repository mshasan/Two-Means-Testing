...
libname abcd 'U:\Documents\My Research (UCF)\SAS 1st paper Final Results\Paper1';
...
s = 10000;  mu1 = 0; sigma1= 1; mu2 = 0; sigma2= 1.5; 
...
denom = sqrt(  l1*l1*((sigma12/n1 + sigma22/n2))  + (1-l1)*(1-l1)*vdiff/n  );
ZUscore = num/denom;

theta = sqrt((sigma12/n1 + sigma22/n2)/(vdiff/n));
alpha =  theta/(1 + theta);
...
RESET STORAGE = "Paper";
	LOAD result;
create abcd.type6 var _All_;
append;
quit;

proc transpose data=abcd.type6 out=abcd.type_6;
run;
