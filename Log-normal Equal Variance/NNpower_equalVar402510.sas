...
*libname abcd 'C:\Research\Shakil\paperfolder';
...
s = 5000;  mu1 = 0; sigma1= 1;   
       m =  0.75 + sqrt(exp(1));    v = (exp(1))*((exp(1))-1) ;  
mu2 = log((m*m)/(sqrt(m*m + v)));
sigma2 = sqrt(log( (m*m + v)/(m*m)));
...
if ((abs(rhoL) < abs(rho)) | (abs(rhoL) =0)) & 
       ( log( 1 + rhoL*(sqrt((exp(sigma1*sigma1) - 1)*(exp(sigma2*sigma2) -1)) ))/(sigma1*sigma2)  < 1) & 
...
x1x2normal = randnormal(n, mean, cov);  *paired sample from bivariate normal;
x1x2[, 1] = exp(x1x2normal[, 1]);
x1x2[,  2] = exp(x1x2normal[, 2]);  *paired sample from bivariate lognormal; 
diff = x1x2[, 1] - x1x2[ , 2]; 
...
create resultdata from result[colname={"n1" "n2" "n" "rhoL" "zupower" "zcpower" "wtpower" "indpower"
"remlpower" "wtpowerU" "ZcpowerU" "ZupowerU" "indpowerU"}] ;
append from result;
close resultdata;
