...
s = 5000;  mu1 = 0; sigma1= 4; mu2 = 0; sigma2= 4; 
...
call randseed(13645);
pr1 = j(n,1); 
call randgen(pr1, "uniform"); 
*rr2= mu2 - sigma2*log((1-u)/u);

call randseed(19645);
pr2 = j(n,1); 
call randgen(pr2, "uniform"); 
*rr2= mu2 - sigma2*log((1-u)/u);

do i = 1 to n;
x1x2[i,1] = mu1 - sigma1*log((1-pr1[i,1])/pr1[i,1]);
x1x2[i,2] = rho*(mu1 - sigma1*log((1-pr1[i,1])/pr1[i,1])) + sqrt(1-rho*rho)*(mu2 - sigma2*log((1-pr2[i,1])/pr2[i,1]));
end;
...
result = result//(n1||n2||n||rho || trho ||  zutype1 ||zctype1 ||wttype1 || indtype1 ||
