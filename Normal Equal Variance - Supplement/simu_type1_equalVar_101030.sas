...
s = 10000;  mu1 = 0; sigma1= 1; mu2 = 0; sigma2= 1; 
...
*calculation of Z-corrected from Looney's paper call it Z_C - equal variance formula;
...
*calculation of Z-corrected from Looney's paper call it Z_CU -unequal variance formula;
newdiff = (n/(n1+n))*x1x2[,1] - (n/(n2+n))*x1x2[, 2];
allx = s1//x1x2[ , 1];  ally = s2//x1x2[ , 2]; 
z_cnum = sum(allx)/(n1+n)  - sum(ally)/(n2+n);
...
*calculations of weighted average of paired and unpaired t- equal variance formula;
...
*calculations of weighted average of paired and unpaired t - unequal variance formula;
...
if htU > ((1-(f- int(f)))*tinv(0.975, int(f)) + (f-int(f))*tinv(0.975, int(f)+1)) |
          htU  < - ((1-(f- int(f)))*tinv(0.975, int(f)) + (f-int(f))*tinv(0.975, int(f)+1))  then wtU[simu,1]=1;
...
result = result//(n1||n2||n||rho ||  zutype1 ||zctype1 || zctype1U ||wttype1 || wttype1U || remltype1) ;
...
create resultdata from result[colname={"n1"  "n2"  "n"   "rho"  "zutype1"  "zctype1"  "zctype1U"  "wttype1" "wttype1U"  
                                                   "remltype1" }] ;
append from result;
close resultdata;
...
PROC EXPORT DATA= resultdata 
            OUTFILE= 'U:\Documents\My Research (UCF)\SAS 1st paper Final Results\Paper\type1_101030.xlsx' 
            DBMS=xlsx 
            REPLACE;
...
RESET STORAGE = "Paper";
	LOAD result;
create abcd.type7 var _All_;
append;
quit;

proc transpose data=abcd.type7 out=abcd.type_7;
run;
