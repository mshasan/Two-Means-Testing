...
s = 10000;  mu1 = 0; sigma1= 1; mu2 = .35; sigma2= 1; 
...
*calculation of Z-corrected from Looney's paper call it Z_C - unequal variance formula;
newdiff = (n/(n1+n))*x1x2[,1] - (n/(n2+n))*x1x2[, 2];
allx = s1//x1x2[ , 1];  ally = s2//x1x2[ , 2]; 
z_cnum = sum(allx)/(n1+n)  - sum(ally)/(n2+n);
...
if ht > ((1-(f- int(f)))*tinv(0.975, int(f)) + (f-int(f))*tinv(0.975, int(f)+1)) |
		  ...
		  *calculations of weighted average of paired and unpaired t- unequal varianc formula;
...
PROC EXPORT DATA= resultdata 
            OUTFILE= 'c:\Research\\Shakil\NNunequal\s101530.xlsx' 
            DBMS=xlsx 
            REPLACE;
...
RESET STORAGE = "Paper";
	LOAD result;
create abcd.power7 var _All_;
append;
quit;

proc transpose data=abcd.power7 out=abcd.power_7;
run;
